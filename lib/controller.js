...
module.exports = function createController() {
  ...
  const collectMiddlewaresMatchingScope = (controller, scope, base = []) => {
    ...
    const inScope = (group) => ~scope.indexOf(group);
    return base.concat(middlewares.filter(mw => mw.scope.some(inScope)));
  }
  ...
  Controller.route = (method, path, action) => {
    method = method.toLowerCase();
    routes.push({ method, path, action, controller: id });
  ...
  Controller.direct = (method, path /* [mw/g], fn */) => {

...
Controller.prototype.middleware = function(scope) {
  if (scope == null) scope = 'all';
  return this.middlewares[scope] || (this.middlewares[scope] = []); 
}

Controller.prototype.route = function(method, path, action) {
  this.routes[action] = { method: method, path: path };
}

Controller.prototype.define = function(name, groups, handler) {
  if (typeof groups === 'function') handler = groups, groups = [];
  this.actions[name] = { groups: groups, handler: handler };
}

Controller.prototype.attach = function(app) {
  var self = this;
